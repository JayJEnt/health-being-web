...
const RecipesSearch: React.FC = () => {
	...
	const [filters, setFilters] = useState<RecipeFilter>(() =>
		Object.keys(filterLabels).reduce(
		...
		async (searchPhrase?: string) => {
			...
			try {
				...
				if (Array.isArray(res)) {
					...
					const shouldLoop =
						res.length < settings.RECIPES_PAGE_SIZE && res.length > 0;
						...
						<div className="px-8">
							<div className="mx-auto max-w-4xl rounded-lg border border-gray-200 bg-gray-50 p-4">
								...
								<div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
									{(Object.keys(filterLabels) as Array<keyof RecipeFilter>).map(
										(filterKey) => (
											<label
												key={filterKey}
			...
			<section className="container mx-auto px-4">
				...
				{!loading && !error && displayedRecipes.length > 0 && (
					<>
						...
						{/* Loading */}
						{loadingMore && (
							<LoadingComponent>Fetching recipes...</LoadingComponent>
						)}
