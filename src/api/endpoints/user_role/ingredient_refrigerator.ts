...
export const ingredientRefrigeratorApi = {
	getById: (ingredient_id: number) =>
		api.get<IngredientRefrigeratorResponse>(
			`${settings.REFRIGERATOR_ENDPOINT}`,
			{ ingredient_id },
		),
	...
	create: (data: IngredientRefrigeratorCreate) =>
		api.post<IngredientRefrigeratorCreateResponse>(
			`${settings.REFRIGERATOR_ENDPOINT}`,
			data,
		),

	delete: (ingredient_id: number) =>
		api.delete<IngredientRefrigeratorDelete>(
			`${settings.REFRIGERATOR_ENDPOINT}`,
			{ ingredient_id },
		),
};
