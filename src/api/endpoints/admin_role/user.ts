...
export const userAdminApi = {
	getByName: (name: string) =>
		api.get<User>(`${settings.USER_ENDPOINT}`, { name }),

	getByEmail: (email: string) =>
		api.get<User>(`${settings.USER_ENDPOINT}`, { email }),

	getById: (user_id: number) =>
		api.get<User>(`${settings.USER_ENDPOINT}`, { user_id }),
	...
	delete: (user_id: number) =>
		api.delete<User>(`${settings.USER_ENDPOINT}`, { user_id }),
};
